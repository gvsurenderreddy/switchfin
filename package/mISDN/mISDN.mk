...
mISDN_SITE=http://svn.astfin.org/software/mISDN-astfin
mISDN_UNZIP=zcat


$(DL_DIR)/$(mISDN_NAME)-$(mISDN_REV):
	$(SVN) -r $(mISDN_REV) $(mISDN_SITE)/$(mISDN_VER) $(DL_DIR)/$(mISDN_NAME)-$(mISDN_REV)
...
ifeq ($(strip $(SF_LEC_128)),y)
	$(MAKE)  LEC_ZL38065=y ENABLE_128MS=y CONFIG_BR4=y CROSS_COMPILE=CROSS_COMPILE=$(TARGET_CROSS) INSTALL_PREFIX=$(UCLINUX_DIR)/linux-2.6.x CC=$(TARGET_CC) LINUX="$(UCLINUX_DIR)/linux-2.6.x" -C $(mISDN_DIR) clean
	...
	$(MAKE)  LEC_ZL38065=y CONFIG_BR4=y CROSS_COMPILE=CROSS_COMPILE=$(TARGET_CROSS) INSTALL_PREFIX=$(UCLINUX_DIR)/linux-2.6.x CC=$(TARGET_CC) LINUX="$(UCLINUX_DIR)/linux-2.6.x" -C $(mISDN_DIR) clean
	...
	$(MAKE)  CONFIG_BR4=y CROSS_COMPILE=CROSS_COMPILE=$(TARGET_CROSS) INSTALL_PREFIX=$(UCLINUX_DIR)/linux-2.6.x CC=$(TARGET_CC) LINUX="$(UCLINUX_DIR)/linux-2.6.x" -C $(mISDN_DIR) clean
	...
	$(MAKE)  CROSS_COMPILE=CROSS_COMPILE=$(TARGET_CROSS) INSTALL_PREFIX=$(UCLINUX_DIR)/linux-2.6.x CC=$(TARGET_CC) LINUX="$(UCLINUX_DIR)/linux-2.6.x" -C $(mISDN_DIR) clean
