...
// TODO: Add define into Makefile
#define ISDN_LIB_DO_NOT_FLIP_BITS /* Do not flip audio bits when using real TDM */
#ifdef ISDN_LIB_DO_NOT_FLIP_BITS
#warning ISDN driver is _NOT_ flipping PCM audio bits
#else
#warning ISDN driver IS FLIPPING PCM audio bits
#endif
...
#define MISDN_GEN_SILENCE
		...
		txfrm->len =len;
		cb_log(9, bc->port, "Transmitting %d SILENCE samples 2 misdn\n", txfrm->len);
...
void manager_ph_control(struct misdn_bchannel *bc, int c1, int c2)
{
	...
	printf("We just called manager_ph_control()\n");
...
void manager_ph_control_block(struct misdn_bchannel *bc, int c1, void *c2, int c2_len)
{
	...
	ctrl->prim = PH_CONTROL | REQUEST;
	printf("We just called manager_ph_control_block \n");
	ctrl->addr = bc->addr | FLG_MSG_DOWN;
	ctrl->dinfo = 0;
...
{
	printf("manager_ec_enable() called....\n");
	struct misdn_stack *stack=get_stack_by_bc(bc);
