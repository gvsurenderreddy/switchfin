...
#define PVT_LOCK(ch)    if (ch)  { ast_mutex_lock(&((ch)->lock)); /*ast_log(LOG_NOTICE,"PVT_LOCK: ch %s", ch->ast ? ch->ast->name : "<NO-AST>");*/   } else
#define PVT_UNLOCK(ch)  if (ch)  { ast_mutex_unlock(&((ch)->lock)); /*ast_log(LOG_NOTICE,"PVT_UNLOCK: ch %s", ch->ast ? ch->ast->name : "<NO-AST>");*/  } else
#define PVT_TRYLOCK(ch) ast_mutex_trylock(&(ch->lock))

#define AST_LOCK(a)   if (a) { ast_mutex_lock(&((a)->lock)); /*ast_log(LOG_NOTICE,"AST_LOCK: ch %s", a->name); */ } else
#define AST_UNLOCK(a) if (a) { ast_mutex_unlock(&((a)->lock)); /*ast_log(LOG_NOTICE,"AST_UNLOCK: ch %s", a->name); */ } else

#define PVT_AST_UNLOCK(c)   \
        PVT_UNLOCK(c);      \
        AST_UNLOCK((c)->ast)
...
static struct chan_list *find_chan_by_bc_lockit(struct chan_list *list, struct misdn_bchannel *bc);
static struct chan_list *find_chan_by_bc_lockall(struct chan_list *list, struct misdn_bchannel *bc);
...
static void release_chan(struct misdn_bchannel *bc, struct chan_list *ch);
...
static int misdn_ast_and_pvt_mutex_lock(struct chan_list *ch)
{
	for (;;) {
		PVT_LOCK(ch);
		if (!ch->ast) {
			PVT_UNLOCK(ch);
			ast_log(LOG_DEBUG, "Unable to lock ch:%p (ast == NULL)\n", ch);
			return -1;
		}
		if (ast_mutex_trylock(&ch->ast->lock)) {
			ast_log(LOG_DEBUG, "Unable to lock ch:%p retrying in 1ms\n", ch);
			PVT_UNLOCK(ch);
			usleep(1000);
			continue;
		}
...
#ifdef CONFIG_HWEC
	if (ec == 1)
		snprintf(bc->pipeline, sizeof(bc->pipeline) - 1, "hwec");
	else if (ec > 1)
		snprintf(bc->pipeline, sizeof(bc->pipeline) - 1, "hwec(deftaps=%d)", ec);
#else
	if (ec == 1)
...
//#endif
	...
	if ( orig  == ORG_AST) {
		...
		if ( !ast_strlen_zero(bc->dad) ) {
                        if (ast->cid.cid_dnid)
                                free(ast->cid.cid_dnid);
                        ast->cid.cid_dnid=strdup(bc->dad);
                }
...
static int misdn_digit_end(struct ast_channel *ast, char digit, unsigned int duration)
{
	...
	switch (p->state) {
		case MISDN_CALLING:
		{
			...
			l = sizeof(ast->exten);
			strncpy(ast->exten, bc->infos_pending, l);
			ast_cdr_update(ast);
		}
		break;
...
static int misdn_fixup(struct ast_channel *oldast, struct ast_channel *ast)
{
	...
	chan_misdn_log(1, p->bc ? p->bc->port : 0,
		       "* IND: Got Fixup State:%s L3id:%x\n",
...
{
	struct chan_list *p       = NULL;
	struct misdn_bchannel *bc = NULL;

	ast_log(LOG_DEBUG, "misdn_hangup(%s)\n", ast->name);

	if (!ast || !(p = MISDN_ASTERISK_TECH_PVT(ast))) {
		chan_misdn_log(3, 0, "misdn_hangup called, without '%s' obj\n",
			       !ast ? "ast_channel" :
			       (!p ? "chan_list" : "???"));
		return -1;
	}
...

{
	...
	if (!ch1) {
		ast_log(LOG_ERROR, "Did not find PVT channel on channel '%s'\n", c0->name);
		return -1;
	}
...
static struct chan_list *init_chan_list(int orig)
{
	...
	if (!cl) {
		chan_misdn_log(-1, 0, "%s: malloc failed!", __FUNCTION__);
		return NULL;
	}
...

{
	...
	if (!cl) {
		ast_log(LOG_ERROR,"Could not create channel list object\n");
		return NULL;
	}
...
{
	struct chan_list *tmp = NULL;

	if (!chan || !text) {
		ast_log(LOG_ERROR, "No channel or text to send\n");
		return -1;
	}
	...
	if (tmp->bc) {
		...
		ast_log(LOG_WARNING, "No B-channel but send_text request?\n");
		PVT_UNLOCK(tmp);
		return -1;
	}
...
static struct chan_list *find_chan_by_bc_lockall(struct chan_list *list, struct misdn_bchannel *bc)
{
	...
	for (;help; help = help->next) {
		if (help->bc == bc) {
			if (misdn_ast_and_pvt_mutex_lock(help)) {
				/* Lock failed: ast channels is being released
				 * Locking PVT only: */
				if (!help->ast) {
					PVT_LOCK(help);
					return help;
...
static struct chan_list *find_holded(struct chan_list *list, struct misdn_bchannel *bc)
{
	...
	for (;help; help=help->next) {
		...
		if ((help->state == MISDN_HOLDED) && (help->hold_info.port == bc->port))
			return help;
	}
...
static struct chan_list *find_holded_lockall(struct chan_list *list, struct misdn_bchannel *bc)
{
	struct chan_list *help = list;
	...
	for (;help; help = help->next) {
		chan_misdn_log(4, bc->port, "$$$ find_holded: --> holded:%d "
			       "channel:%d\n",help->state==MISDN_HOLDED,
			       help->hold_info.channel);
...
static struct chan_list *find_holded_l3_lockall(struct chan_list *list, unsigned long l3_id, int w)
{
	struct chan_list *help = list;

	for (;help; help = help->next) {
		if ((help->state == MISDN_HOLDED) && (help->l3id == l3_id)) {
			if (misdn_ast_and_pvt_mutex_lock(help)) {
				/* Lock failed: ast channels is being released
				 * Locking PVT only: */
				if (!help->ast) {
...
/* A valid BC _or_ a chan_list pointer should be given: if chan_list is given
 * it must be already locked by caller
 */
static void release_chan(struct misdn_bchannel *bc, struct chan_list *ch) {
	struct ast_channel *ast = NULL;

	if (!bc && !ch) {
		chan_misdn_log(1, bc->port, "release_chan: Ch and bc are NULL!\n");
		return;
	}

	if (ch == NULL) {
		ch = find_chan_by_bc_lockit(cl_te, bc);
	}
	...
	ch->ast = NULL;
	/* Let possible thread waiting for the channel to clear */
	ch->state = MISDN_CLEANING;
	PVT_UNLOCK(ch);
	PVT_LOCK(ch);
	PVT_UNLOCK(ch);
	ast_mutex_destroy(&ch->lock);
	free(ch);
}
/*** release end **/
...
cb_events(enum event_e event, struct misdn_bchannel *bc, void *user_data)
{
	...
	case EVENT_NEW_BC:
		if (!ch) {
			ch = find_holded_lockall(cl_te, bc);
		}
	...
	case EVENT_DTMF_TONE:
		{
			...
			if (misdn_ast_and_pvt_mutex_lock(ch)) {
				ast_log(LOG_WARNING, "Unable to lock channel: ignoring DTMF:%c\n", bc->dtmf);
				break;
			}
	...
	case EVENT_INFORMATION:
		{
			int l;
			if (misdn_ast_and_pvt_mutex_lock(ch)) {
				ast_log(LOG_WARNING, "Unable to lock channel: ignoring INFORMATION event\n");
				break;
			}
	...
	case EVENT_SETUP:
		{
			struct chan_list *ch = find_chan_by_bc_lockall(cl_te, bc);
			if (ch) {
				switch (ch->state) {
				case MISDN_NOTHING:
					PVT_AST_UNLOCK(ch);
...

		{
			...
			/* Lock before queuing the channel */
			misdn_ast_and_pvt_mutex_lock(ch);
	...
	case EVENT_SETUP_ACKNOWLEDGE:
		{
			if (misdn_ast_and_pvt_mutex_lock(ch)) {
				ast_log(LOG_WARNING, "Unable to lock channel: ignoring SETUP ACK. event\n");
				break;
			}
			ch->state = MISDN_CALLING_ACKNOWLEDGE;
	...
	case EVENT_PROCEEDING:
		{
			...
			if (misdn_ast_and_pvt_mutex_lock(ch)) {
				ast_log(LOG_WARNING, "Unable to lock channel: ignoring PROCEEDING event");
				break;
			}
	...
	case EVENT_PROGRESS:
		...
		if (misdn_ast_and_pvt_mutex_lock(ch)) {
			ast_log(LOG_WARNING, "Unable to lock channel: ignoring PROGRESS event");
			break;
		}
	...
	case EVENT_ALERTING:
		{
			if (misdn_ast_and_pvt_mutex_lock(ch)) {
				ast_log(LOG_WARNING, "Unable to lock channel: ignoring ALERTING event");
				break;
			}
	...
	case EVENT_CONNECT:
		{
			...
			if (misdn_ast_and_pvt_mutex_lock(ch)) {
				ast_log(LOG_WARNING, "Unable to lock channel: ignoring CONNECT event");
				break;
			}
	...
	case EVENT_CONNECT_ACKNOWLEDGE:
		{
			if (misdn_ast_and_pvt_mutex_lock(ch)) {
				ast_log(LOG_WARNING, "Unable to lock channel: ignoring CONNECT ACK. event");
				break;
			}
	...
	case EVENT_DISCONNECT:
		...
		if (ch) {
			...
			if (misdn_ast_and_pvt_mutex_lock(ch)) {
				ast_log(LOG_WARNING, "Unable to lock channel: ignoring DISCONNECT event");
				break;
			}
		...
		/*	ch = find_holded_l3_lockall(cl_te, bc->l3_id,1);
			if (ch) {
				hangup_chan(ch);
			}
		*/
	...
	case EVENT_RELEASE:
		if (!ch) {
			ast_log(LOG_WARNING, "ch is NULL on RELEASE event");
			release_chan(bc, NULL);
			break;
		}

		/* Usually ast_channel is NULL here */
		if (ch->ast) {
			misdn_ast_and_pvt_mutex_lock(ch);
		} else {
			PVT_LOCK(ch);
			...
			if (ch) {
				if (misdn_ast_and_pvt_mutex_lock(ch)) {
					if (!ch->ast) {
						...
						ast_log(LOG_WARNING, "Unable to lock channel: ignoring RELEASE COMPLETE event");
						break;
					}
				}
				stop_bc_tones(ch);
	...
	case EVENT_CLEANUP:
		if (misdn_ast_and_pvt_mutex_lock(ch)) {
			if (!ch->ast) {
				...
				ast_log(LOG_WARNING, "Unable to lock channel: ignoring B-CH ERROR / CLEANUP event");
				break;
			}
		}
	...
	case EVENT_TONE_GENERATE:
		if (misdn_ast_and_pvt_mutex_lock(ch)) {
			ast_log(LOG_WARNING, "Unable to lock channel: ignoring TONE GENERATE event");
			break;
		}
	...
	case EVENT_BCHAN_DATA:
		/* No locking for B-channel audio stream data */
				...
				if (ret <= 0) {
					...
					if (misdn_ast_and_pvt_mutex_lock(ch)) {
						if (!ch->ast) {
							...
							ast_log(LOG_WARNING, "Unable to lock channel: ignoring B-CH event");
							break;
						}
					}
	...
	case EVENT_TIMEOUT:
		if (misdn_ast_and_pvt_mutex_lock(ch)) {
			ast_log(LOG_WARNING, "Unable to lock channel: ignoring TIMEOUT event");
			break;
		}
	...
	case EVENT_RETRIEVE:
		{
			...
			if (!ch) {
				...
				if (misdn_ast_and_pvt_mutex_lock(ch)) {
					ast_log(LOG_WARNING, "Unable to lock channel: ignoring RETRIEVE event");
					break;
				}
			}
			if (!ch) {
	...
	case EVENT_HOLD:
		{
			...
			if (bridged) {
				if (misdn_ast_and_pvt_mutex_lock(ch)) {
					ast_log(LOG_WARNING, "Unable to lock channel: ignoring HOLD event");
					break;
				}
	...
	case EVENT_FACILITY:
		...
		if (misdn_ast_and_pvt_mutex_lock(ch)) {
			ast_log(LOG_WARNING, "Unable to lock channel: ignoring FACILITY event");
			break;
		}
		...
		case Fac_CD:
			{
				...
				if (bridged && MISDN_ASTERISK_TECH_PVT(bridged)) {
					...
					if (misdn_ast_and_pvt_mutex_lock(ch_br)) {
						ast_log(LOG_WARNING, "Unable to lock bridged channel: ignoring FACILITY event");
						PVT_AST_UNLOCK(ch);
						break;
					}

					if (ch_br->bc) {
						if (ast_exists_extension(bridged,
									 ch->context, (char *)bc->fac_in.u.CDeflection.DeflectedToNumber,
									 1, bc->oad)) {
							ch_br->state = MISDN_DIALING;
							PVT_AST_UNLOCK(ch_br);
							PVT_AST_UNLOCK(ch);
		...
		break; //DPN: We don't want to restart after each facility event		
	...
	case EVENT_RESTART:
		...
		if (!bc->dummy) {
			...
			if (misdn_ast_and_pvt_mutex_lock(ch)) {
				ast_log(LOG_WARNING, "Unable to lock channel: ignoring RESTART event");
				break;
			}
			ast = ch->ast;
	...
	default:
		chan_misdn_log(1, bc ? bc->port : 0, "Got Unknown Event %#x\n", event);
		break;
	}
...
struct misdn_jb *misdn_jb_init(int size, int upper_threshold)
{
    ...
    memset(jb->ok, 0, size * sizeof(char));

    ast_mutex_init(&jb->mutexjb);

#ifdef CHAN_MISDN_JB_DEBUG
    counter++;
    chan_misdn_log(1,0,"jb_init. alloc #%d\n", counter);
#endif
    return jb;
}
...
#ifdef CHAN_MISDN_JB_DEBUG
	counter--;
	chan_misdn_log(1,0,"jb_free. free #%d\n", counter);
#endif
}
