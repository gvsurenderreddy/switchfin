...
char CallerID[60]=""; //Keep the callers ID of the incoming GSM calls.
		      //This variable is exported to Asterisk using the proc. 
		      //Can be used only diring active call. 
...
static void fx_write_WM8510(struct wcfxs *wc,int port, int value)
{
 sport_configure_gsm(SPI_BAUDS_GSM); //Configure the SPI (sport) to 1.33MHz/16bits
...
void  GSM_receive(struct wcfxs *wc,int port, char *rcv_str)
{
	u16 reg,i,temp;
	int keep_s;
	...
	if(reg==0) {//if no data to read -> output 'x' string
		rcv_str[0]='x';
		rcv_str[1]=0x00;//string terminating character
		return;
	}
	...
	keep_s=0;
	if(reg>64)
		printk("receiving buffer overflow!");
...
void Extract_CallerID(char *clcc, char *CID){
	...
	i=0; commas=0;
	while (commas<5 && clcc[i])		//Count 5 commas
		if(clcc[i++]==',') commas++;
	
	if (commas!=5) return;

	while ( clcc[i] != '"' && clcc[i]) {    //Serach for "
		if(clcc[i++]==',') break;
	}
	...
	j=0;
	while ( clcc[i] != '"' && clcc[i]) {    //Copy CID 

		CID[j++]=clcc[i++];
	}
	
	
	if(clcc[i]=='"') 			//Check for closing "
		CID[j]=0;
	else
		CID[0]=0;				
...
//Read one SMS from the GSM module if available, deletes it from teh module
//If no message available -1 is returned
...
static int sms_read(struct wcfxs *wc,int port, char **message) {
	...
	if(sms_max != 32) {
        ...
        if(sms_count < 0 || sms_count > sms_max) {
		printk("Strange amount of SMS %d\n", sms_count);
		return -1;
	}
	...
	if(sms_count){	//We have SMS available lets read one
		...
		//Let's alocate memory and copy the message
        	*message = kmalloc(len+1, GFP_ATOMIC);
        	if (!*message) {
                	printk("Can not allocate memory for the message\n");
                	return -1;
        	}
        	strcpy(*message, rcv_str);
		...
		if(debug>=2)
			printk("No message available!\n");
                return -1;
	}
}


//Sends SMS to the specified number. message is expected in the form
//number:text 
//In case of success 0 is returned -1 otherwise
static int sms_send(struct wcfxs *wc,int port, char *message) {

        char cmd_str[SMS_MAX_LEN], rcv_str[50];
	...
	for(i=0;i<SMS_NUM_MAX_LEN; i++){
		...
		if(message[i] != '+' && (message[i]<'0' || message[i]>'9') ) break; //If not valid number symbol
	}
...
//Dialing part
	...
	if(outgoing_call_state==1){
		GSM_receive(wc, card, rcv_str);
		if(strcmp(rcv_str,"x")==0) return;
		if(strcmp(rcv_str,"0")==0){
	...
	if(outgoing_call_state==2){
		...
		if(strcmp(rcv_str,"3")==0){
			outgoing_call_state=0;
			dahdi_hooksig(&wc->chans[card], DAHDI_RXSIG_ONHOOK);
			if (debug>=2)
				printk("NO CARRIER during conversation\n");
			*CallerID=0;
			return;
		}
		//If BUSY received, initialize the state machine
		//Inform zaptel for call disconnect
		if(strcmp(rcv_str,"7")==0){
			outgoing_call_state=0;
			dahdi_hooksig(&wc->chans[card], DAHDI_RXSIG_ONHOOK);
			if (debug>=2)
				printk("BUSY %s\n",rcv_str);
...
//Receiving a call
	...
	if(incomming_call_state==0){
			...
			if(strcmp(rcv_str,"2")==0){
				 interrupt_count=2;
				 if (debug>=2)
					 printk("Ring: %s\n",rcv_str);
			}
			else{
				if(strcmp(rcv_str,"x")!=0){
					if (debug>=2)
						printk("ics=%d str_rec=%s\n",incomming_call_state,rcv_str);
				}
			}
			if(interrupt_count==2){
				...
				if (debug>=2)
					printk("dahdi_hooksig DAHDI_RXSIG_RING sent!\n");
                                ...
                                //Retreive the incomming call Caller ID
                                if(!(*CallerID)){
					GSM_send(wc, card, "at+clcc");
                                	wait_just_a_bit(HZ/3);
                                	GSM_receive(wc, card, rcv_str);
					Extract_CallerID(rcv_str, CallerID);
                                	if(debug>=2) 
						printk("GSM1 CallerID: %s\n", CallerID);
				}
			...
			if(interrupt_count==1) {
				dahdi_hooksig(&wc->chans[card], DAHDI_TXSIG_OFFHOOK);
				if (debug>=2)
					printk("dahdi_hooksig DAHDI_TXSIG_OFFHOOK sent!\n");
				interrupt_count=0;
	...
	if(incomming_call_state==1){
			...
			if(strcmp(rcv_str,"3")==0){
				...
				dahdi_hooksig(&wc->chans[card],DAHDI_TXSIG_ONHOOK);
				if (debug>=2)
					printk("NO CARRIER during ringing\n");
				*CallerID=0;
				return;
			}
			//When RING is received
	...
	if(incomming_call_state==2){
		...
		if(strcmp(rcv_str,"3")==0){
			...
			if (debug>=2){
				printk("NO CARRIER during conversation\n");
				printk("dahdi_hooksig DAHDI_TXSIG_ONHOOK sent!\n");
			}
			*CallerID=0;
			return;
		}
...
//GSM management part
	...
	if((reg & 0x0002)!=0){
		...
		GSM_receive(wc, port, rcv_str);
		if (debug)
			printk("Received: %s\n",rcv_str);
		} while(strcmp(rcv_str,"NORMAL POWER DOWN")!=0 && (--cntr>0));
	...
	do {
		...
		if (debug)
			printk("Received: %s\n",rcv_str);
	} while(strcmp(rcv_str,"OK")!=0 && (--cntr>0));
	...
	do {
		...
		if (debug)
			printk("Received: %s\n",rcv_str);
	} while(strcmp(rcv_str,"0")!=0 && (--cntr>0));
	...
	if(strcmp(rcv_str,"+CPIN: SIM PIN")==0)
	{
		...
		if(strcmp(rcv_str,"+CME ERROR: incorrect password")==0)
		{
	...
	{
		//Card not inserted
		if(strcmp(rcv_str,"+CME ERROR: SIM not inserted")==0)
		{
		...
		{
			//Card unlocked
			if(strcmp(rcv_str,"+CPIN: READY")==0)
			{
	...
	if(debug)
		printk("Wait for GSM init ");
	cntr=60;
	do{
		wait_just_a_bit(HZ);
		GSM_send(wc,port,"at+ccalr?");	
		wait_just_a_bit(HZ/10);
		GSM_receive(wc, port, rcv_str);
...
{	
	...
	fx_write_WM8510(wc,port,0x000F); 
	...
	// 8bits= 0x02 + 8bits = 0x0B (set BIASEN=1,BUFIOEN=1,VMIDSEL=11)
        fx_write_WM8510(wc,port,0x020F);

	//Starting the ALC function;Register 0x20; ALCSEL;max gain
        fx_write_WM8510(wc,port,0x4138);

	//DAC volume. Register 0x0B; 
        fx_write_WM8510(wc,port,0x16D0); //was 0x16F0

	//OSR enable=128 OSR;Register 0x0E;set ADCOSR, High pass filter enabled
        fx_write_WM8510(wc,port,0x1D08);

        //Register 0x21;
        fx_write_WM8510(wc,port,0x420B); //ALCLVL -> -7.5dB relative to Full Scale
					 //ALC hold time 0 ms
					 //zero crossing disable		


	//Noise gate enable;Register 0x22;set ALCMODE->limiter!
        fx_write_WM8510(wc,port,0x4532);   //ALCMODE->limiter!
