...
int main(int argc, char *argv[])
{
	...
	strip_lines("/persistent/etc/zaptel.conf","fxoks=","fxsks=");
	strip_lines("/persistent/etc/asterisk/zapata.conf","signalling =","channel =>");

	zdc = fopen("/persistent/etc/zaptel.conf", "a");
	zsc = fopen("/persistent/etc/asterisk/zapscan.conf", "w");
	zap = fopen("/persistent/etc/asterisk/zapata.conf", "a");
	if (fd < 0) {
...
void strip_lines(const char* filename, const char* text1, const char* text2)
{
	...
	if (fp && fptemp)
	{
	   ...
	   *begin with text1 or text2 */
	   ...
	   if( result < 0){
	      printf("using system command %s to copy file failed!", cmd);
	   }
	}
	else
	{
