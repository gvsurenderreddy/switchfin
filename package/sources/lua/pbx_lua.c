...
//static struct ast_hashtab *local_table = NULL; //DPN don't use has table
...
/*! DPN: copied from ast 1.6 main/datastore.c
 *
 ...
 */
struct ast_datastore *__ast_datastore_alloc(const struct ast_datastore_info *, const char *, const char *, int, const char *);
...
/*! DPN: copied from ast 1.6 include/asterisk/datastore.h
...
/*! DPN: copeid from ast 1.6 main/pbx.c
...
/*! DPN: copied from ast 1.6 main/pbx.c
 * \brief the full routine to compare extensions in rules.
 */
static int ast_extension_cmp(const char *a, const char *b)
{
...
static int lua_pbx_exec(lua_State *L)
{
	...
	if (option_verbose > 2) { 
		ast_verbose(VERBOSE_PREFIX_3 "Executing [%s@%s:%d] %s(\"%s\", \"%s\")\n",
...
//	if (!local_table) DPN: comment the has table 
...
//		con = ast_context_find_or_create(&local_contexts, local_table, context_str, registrar); DPN: remove hastab
		con = ast_context_find_or_create(&local_contexts, context_str, registrar);
		if (!con) {
...
//	ast_merge_contexts_and_delete(&local_contexts, local_table, registrar); //DPN don't use has table
...
//	local_table = NULL; DPN comment the has table
	local_contexts = NULL;
...
static int lua_find_extension(lua_State *L, const char *context, const char *exten, int priority, ast_switch_f *func, int push_func)
{
	...
	if (option_debug>1)
		ast_log(LOG_DEBUG, "Looking up %s@%s:%i\n", exten, context, priority);
	...
	for (i = 1; i < lua_objlen(L, context_order_table) + 1; i++) {
		...
		if (func == &matchmore)
			//match = ast_extension_close(e, exten, E_MATCHMORE); //DPN
			match = ast_extension_close(e, exten, 1); 

		else if (func == &canmatch)
			//match = ast_extension_close(e, exten, E_CANMATCH); //DPN
			match = ast_extension_close(e, exten, 1);
		else
...
AST_MODULE_INFO(ASTERISK_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS, "Lua PBX Switch", //DPN
		.load = load_module,
